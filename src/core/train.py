...
def train(run_folder: str, output_dir: str, is_lora: bool):
    import torch
    ...
    if is_lora:
        ...
        with open(f"{run_folder}/logs.txt", "a") as f:
            f.write(
                f"<br>merge: https://modal.com/logs/call/{merge_handle.object_id}\n"
            )
            print(f"Beginning merge {merge_handle.object_id}.")
        return merge_handle
    else:
        print("Full fine-tuning complete. No LoRA merge needed.")
        return None
...
def launch(config_raw: dict, data_raw: str, run_to_resume: str, preproc_only: bool):
    ...
    is_lora = "adapter" in config and config["adapter"] in ["lora", "qlora"]
    ...
    print(f"Preparing {training_type} fine-tuning run in {run_folder}.")
    ...
    if preproc_only:
        ...
        launch_handle = train.spawn(run_folder, config["output_dir"], is_lora)
...
def main(
    ...
    if merge_handle and merge_lora and not preproc_only:
        merge_handle.get()
